...
function WhitespaceTab() {
  ...
  function icpVerdict(): { text: string; desc: string; color: string } | null {
    ...
    if (passedCount === 6 && failedCount === 0)
      return {
        text: 'Qualifies as ARICP target',
        desc: 'All 6 criteria met. This account qualifies for the ARwhitespace motion. Proceed to the 8-question discovery guide below.',
        color: 'var(--state-success)',
      };
    if (failedCount > 0)
      return {
        text: 'Does not qualify — criteria not met',
        desc: 'One or more required criteria are not met. Do not run the ARwhitespace motion on this account. Log for future review at renewal or when spend grows.',
        color: 'var(--danger)',
      };
    return {
      text: 'Partial — finish assessing',
      desc: 'Keep marking criteria to get a full qualification read.',
  ...
  const scoringTiers = [
    {
      ...
      action: 'Schedule ARdiscovery immediately',
      desc: "You have enough signal. Get the ARAdvisory Director on the call and lock in next steps before the conversation ends. Don't wait.",
    },
    {
      dot: 'var(--warning-amber)',
      label: '1–2 Green Flags',
      labelColor: 'var(--warning-amber)',
      action: 'Probe one more level — worth a short intro demo',
      desc: "There's signal but not full conviction yet. Ask one follow-up, then offer to connect them with the ARteam for a short no-pressure intro.",
    },
    {
      dot: 'var(--danger)',
      label: '0 Green Flags',
      labelColor: 'var(--danger)',
      action: 'Park it — revisit at renewal',
      desc: "Don't force it. Log for a future trigger — renewal, re-org, new champion — and move on. A bad-fit conversation hurts your credibility as a CD champion.",
    },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 'var(--space-lg)' }}>
        <div className={styles.sectionEyebrow}>ARWhitespace Program · Step 1</div>
        <h2 className={styles.sectionTitle}>Qualify the account. Run the discovery.</h2>
        <p className={styles.sectionSub}>
          Use this before any ARconversation with an existing CI or CD customer. First check the ICP
          criteria — if they qualify, work through the 8 discovery questions to gauge readiness and
          surface pain.
        </p>
      </div>
      ...
      >
        ...
        <span>
          New reps or new logo pursuits. This is a cross-sell motion only for existing CI/CD/IaCM
          customers with account context and relationship history.
        </span>
      </div>
      ...
      >
        <div className={styles.sectionEyebrow} style={{ marginBottom: 0 }}>
          ICP Qualification Checklist
        </div>
        <span style={{ fontSize: 'var(--text-label)', color: 'var(--text-muted)' }}>
          — Mark each criterion met or not met
        </span>
        <button
          onClick={() => setIcpState({})}
          style={{
          ...
          return (
            <div key={i} className={styles.csIcpCard} style={{ borderColor, background: bg }}>
              <div className={styles.csIcpNum}>
                <span
                  className={styles.csIcpNumBadge}
                  style={{
                    background: `color-mix(in srgb, ${c.threshColor} 12%, transparent)`,
                    color: c.threshColor,
                    border: `1px solid color-mix(in srgb, ${c.threshColor} 25%, transparent)`,
                  }}
                >
                  {c.num}
                </span>
                {st === true && (
                  <span style={{ color: 'var(--state-success)', fontSize: 'var(--text-body-sm)' }}>
                    ✓
                  </span>
                )}
                {st === false && (
                  <span style={{ color: 'var(--danger)', fontSize: 'var(--text-body-sm)' }}>✗</span>
                )}
              </div>
              <div className={styles.csIcpTitle}>{c.title}</div>
              <div className={styles.csIcpDesc}>{c.desc}</div>
              <div
                className={styles.csIcpThreshold}
                style={{
                  background: `color-mix(in srgb, ${c.threshColor} 10%, transparent)`,
                  color: c.threshColor,
                  border: `1px solid color-mix(in srgb, ${c.threshColor} 25%, transparent)`,
                }}
              >
              ...
              <div className={styles.csIcpToggle}>
                <button
                  ...
                  style={{
                    borderColor: 'color-mix(in srgb, var(--state-success) 40%, transparent)',
                    color: 'var(--state-success)',
                    background: st === true ? 'var(--state-success-bg)' : 'transparent',
                  }}
                ...
                <button
                  ...
                  style={{
                    ...
                    color: 'var(--danger)',
                    background: st === false ? 'var(--danger-bg)' : 'transparent',
                  }}
      ...
      {verdict && (
        <div className={`${styles.csIcpResult} ${styles.csIcpResultShow}`}>
          <div className={styles.csIcpScoreBig} style={{ color: verdict.color }}>
            {passedCount}/6
          </div>
          <div>
            <div className={styles.csIcpVerdict} style={{ color: verdict.color }}>
              {verdict.text}
            </div>
      ...
      >
        <div className={styles.sectionEyebrow} style={{ marginBottom: 0 }}>
          Discovery Guide — 8 Questions
        </div>
        <span style={{ fontSize: 'var(--text-label)', color: 'var(--text-muted)' }}>
          — Click a flag on each question to record your read
        </span>
        <button
          onClick={() => {
            setQFlags({});
      ...
      >
        For CI/CD champion conversations — surface pain naturally, don't pitch a product. After 2–3
        questions you should have a clear sense of whether to schedule a dedicated ARdiscovery call.
      </div>
      ...
      <div className={styles.csDiscoLayout}>
              ...
              {yellows > 0 && (
                <span
                  ...
                  style={{
                    ...
                    color: 'var(--warning-amber)',
                    border: '1px solid color-mix(in srgb, var(--warning-amber) 30%, transparent)',
                  }}
                >
                  {yellows} ~
                </span>
          ...
          {DISCO_QUESTIONS.map((dq, i) => {
            ...
            return (
              ...
              >
                ...
                {flagToken && (
                  <div className={styles.csDqNavFlag} style={{ background: flagToken.dot }} />
                )}
              </button>
            );
          })}
        ...
        <div className={styles.csDiscoPanel}>
          ...
          <div className={styles.csFlagCards}>
            {(['green', 'yellow', 'red'] as FlagColor[]).map((fk) => {
              ...
              return (
                ...
                >
                  <div
                    className={styles.csFlagCardHdr}
                    style={{ background: isSelected ? ft.bg : 'var(--bg-panel)' }}
                  >
                    <div className={styles.csFlagDot} style={{ background: ft.dot }} />
                    <span className={styles.csFlagLabel} style={{ color: ft.label }}>
                      {flagData.label}
                    </span>
                    <span className={styles.csFlagAction} style={{ color: ft.label }}>
                      {flagData.action}
                    </span>
                    {isSelected && (
                      <span
          ...
          <div className={styles.csDqBottom}>
            ...
            >
              <div className={styles.csDqSolvesLbl}>ARsolves with</div>
              <div className={styles.csDqSolves} style={{ marginTop: 'var(--space-sm)' }}>
                {q.solves.map((s) => (
                  <span key={s} className={styles.csDqSolveTag}>
                    {s}
                  </span>
                ))}
              </div>
      ...
      <div style={{ marginTop: 'var(--space-xl)' }}>
        ...
        >
          <div className={styles.sectionEyebrow} style={{ marginBottom: 0 }}>
            Scoring — What to do after the conversation
          </div>
          {allAnswered && (
            <span
              style={{
          ...
          {!allAnswered && Object.keys(qFlags).length > 0 && (
            <span style={{ fontSize: 'var(--text-label)', color: 'var(--text-muted)' }}>
              ({Object.keys(qFlags).length} of 8 answered — finish all 8 to see your result)
            </span>
          )}
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderColor: isActive
                    ? i === 0
                      ? 'color-mix(in srgb, var(--state-success) 50%, transparent)'
                      : i === 1
              ...
              >
                <div className={styles.csScTierHdr}>
                  ...
                  <div className={styles.csScTierLabel} style={{ color: tier.labelColor }}>
                    {tier.label}
                  </div>
                </div>
                <div className={styles.csScTierAction} style={{ color: tier.labelColor }}>
                  {tier.action}
                </div>
        ...
        >
          Note: Q7 and Q8 are related but distinct. Q7 surfaces cross-functional ownership gaps; Q8
          confirms whether a formal FinOps role exists. Use Q7 first and only go to Q8 if the answer
          is ambiguous.
        </div>
      </div>
    </div>
...
function MotionCard({
  ...
  return (
    ...
    >
      <div
        className={styles.csFcBanner}
        style={{ background: `color-mix(in srgb, ${m.color} 8%, transparent)` }}
      >
        <div
          className={styles.csFcIcon}
          style={{
        ...
        <div>
          <div className={styles.csFcName} style={{ color: m.color }}>
            {m.name}
          </div>
          <div className={styles.csFcTagline} style={{ color: m.color }}>
            {m.tagline}
          </div>
        </div>
      </div>
      <div className={styles.csFcBody}>
        ...
        <div className={styles.csFcTargets}>
          {m.targets.map((t, i) => (
            <span
              key={t}
              className={styles.csFcTarget}
              style={{
                color: m.targetColors[i],
                background: `color-mix(in srgb, ${m.targetColors[i]} 10%, transparent)`,
                borderColor: `color-mix(in srgb, ${m.targetColors[i]} 25%, transparent)`,
              }}
            >
              {t}
            </span>
    ...
    <div className={`${styles.csDetailPanel} ${styles.csDetailPanelOpen}`}>
      ...
      >
        <div>
          ...
          <div className={styles.csDpTagline} style={{ color: m.color }}>
            {m.tagline}
          </div>
        </div>
        <button className={styles.csDpClose} onClick={onClose}>
          Close ✕
        </button>
      </div>
      <div className={styles.csDpBody}>
        <div className={styles.csDpCol}>
          <div className={styles.csDpColLabel} style={{ color: m.color }}>
            {m.expand.leftLabel}
          </div>
          {m.expand.leftItems.map((item) => (
            <div key={item} className={styles.csDpItem}>
              {item}
            </div>
          ))}
        </div>
...
function UpsellTab() {
  ...
  const activeMotion = activeMotionId
    ? (allMotions.find((m) => m.id === activeMotionId) ?? null)
    : null;
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 'var(--space-lg)' }}>
        ...
        <h2 className={styles.sectionTitle}>Harness ARUpsell Playbook</h2>
        <p className={styles.sectionSub}>
          Every module you sell alongside ARmultiplies the customer's value — and your deal size.
          Use this guide to identify the right upsell motion, the right persona, and the exact
          conversation starter for each scenario.
        </p>
      </div>
      ...
      <div className={styles.qaSkuTabs} style={{ marginBottom: 'var(--space-lg)' }}>
        {(['motions', 'pathways', 'maturity', 'starters'] as UpsellTab[]).map((t) => {
          const labels: Record<UpsellTab, string> = {
            motions: 'Upsell Motions',
            pathways: 'Savings Pathways',
            maturity: 'By Maturity Stage',
        ...
        <div>
          ...
          <div className={styles.csRowSection}>
            <div className={styles.csRowLabel}>
              <span
                className={styles.csRowLabelPill}
                style={{
              ...
              <span className={styles.csRowLabelHint}>
                — click any card for full details &amp; conversation starters
              </span>
            </div>
            <div className={styles.csFlowGrid}>
              {CCM_MOTIONS.map((m) => (
                <MotionCard
                  key={m.id}
                  m={m}
                  isSelected={activeMotionId === m.id}
                  onToggle={() => toggleMotion(m.id)}
                />
              ))}
            </div>
          </div>
          ...
          <div className={styles.csRowSection}>
            <div className={styles.csRowLabel}>
              <span
                className={styles.csRowLabelPill}
                style={{
                  background: 'var(--sku-ag-bg)',
                  border: '1px solid var(--sku-ag-bdr)',
                  color: 'var(--color-devops)',
                }}
              >
                Other Harness Modules
              </span>
              <span className={styles.csRowLabelHint} style={{ color: 'var(--color-devops)' }}>
                — click any card for full details &amp; conversation starters
              </span>
            </div>
            <div className={styles.csFlowGrid}>
      ...
      {upsellTab === 'pathways' && (
        <div>
          <div className={styles.sectionEyebrow} style={{ marginBottom: 'var(--space-sm)' }}>
          ...
          <div className={styles.csPathwayTitle}>
            Recommended upsell sequence — {currentPathway.label}
          </div>
          ...
          <div className={styles.csPathSteps}>
            {currentPathway.steps.map((step, i) => (
              <>
                {i > 0 && (
                  <div key={`arrow-${i}`} className={styles.csPathArrow}>
                    →
                  </div>
                )}
                <div key={step.name} className={styles.csPathStep}>
                  <div
                    className={styles.csPathStepInner}
                    style={{ borderTopColor: stepColors[i % stepColors.length] }}
                  >
                    <div
                      className={styles.csPathStepNum}
                    ...
                    <div
                      className={styles.csPathStepVal}
                      style={{ color: stepColors[i % stepColors.length] }}
                    >
                      {step.val}
                    </div>
      ...
      {upsellTab === 'maturity' && (
        <div>
          <div className={styles.csMatGrid}>
            {MATURITY_UPSELL.map((stage) => (
              <div key={stage.num} className={styles.csMatCol}>
                ...
                <div className={styles.csMatStageLabel} style={{ color: stage.labelColor }}>
                  {stage.label}
                </div>
                <div className={styles.csMatSkuCards}>
                  {stage.skus.map((sku) => (
                    <div
                      key={sku.name}
                      className={styles.csMatSkuCard}
                      style={{ borderTopColor: sku.color }}
                    >
                      <div className={styles.csMscHeader}>
                        <span className={styles.csMscIcon}>{sku.icon}</span>
                        <span className={styles.csMscName} style={{ color: sku.color }}>
                          {sku.name}
                        </span>
                        <span
                          className={styles.csMscTag}
                          style={{
                            background: `color-mix(in srgb, ${sku.tagColor} 10%, transparent)`,
                            color: sku.tagColor,
                            border: `1px solid color-mix(in srgb, ${sku.tagColor} 25%, transparent)`,
                          }}
                        >
                          {sku.tag}
                        </span>
          ...
          <div className={styles.csPitchBox}>
            ...
            <div className={styles.csPitchText}>
              "Let me show you where you are on the FinOps maturity curve — and which Harness SKUs
              get you to the next stage. Most teams leave $2–5M on the table by staying at Crawl."
            </div>
          </div>
        </div>
      )}
            ...
            {filteredStarters.map((s, i) => (
              ...
              >
                <div className={styles.qaCardHdr}>
                  ...
                  <span
                    className={`${styles.qaArrow} ${openStarters[i] ? styles.qaArrowOpen : ''}`}
                  >
                    ▼
                  </span>
                </div>
  ...
  return (
    <div>
      <div className={styles.sectionHdr}>
        <div className={styles.sectionEyebrow}>ARSales Playbook · 2026</div>
        <h2 className={styles.sectionTitle}>Cross-Sell Playbook</h2>
        <p className={styles.sectionSub}>
          Identify ARwhitespace in existing accounts and convert it to pipeline. Step 1 qualifies
          and discovers. Step 2 maps the right modules to close.
        </p>
      </div>
