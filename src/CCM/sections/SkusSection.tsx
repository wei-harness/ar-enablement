...
export function SkusSection() {
  ...
  const roiVals = useMemo(() => sku.roi.compute(slider1, slider2), [sku, slider1, slider2]);
  ...
  return (
    <div>
      <div className={styles.sectionHdr}>
        ...
        <h2 className={styles.sectionTitle}>ARSKUs</h2>
        <p className={styles.sectionSub}>
          Select a SKU to explore positioning, features, discovery questions, and ROI calculator.
        </p>
      </div>
          ...
          >
            <div className={styles.skuCardHdr}>
              <div
                ...
                style={{ background: s.bg, border: `1px solid ${s.bdr}`, color: s.color }}
              >
                <CCMIcon name={s.icon} size={18} />
              </div>
              <div>
            ...
            <div className={styles.skuValue} style={{ color: s.color }}>
              {s.value}
            </div>
      ...
      {activeTab === 'pitch' && (
        <div>
          ...
          <div className={styles.statGrid}>
            {sku.stats.map((s) => (
              <div key={s} className={styles.statsItem}>
                {s}
              </div>
            ))}
          </div>
        ...
        <div className={styles.featureList}>
          {sku.useCases.map((uc) => (
            <div key={uc} className={styles.featureItem}>
              <div className={styles.featureCheck}>→</div>
              <div>
                <div className={styles.featureName}>{uc}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={styles.discoveryList}>
          {sku.discovery.map((d, i) => (
            ...
            >
              ...
              <div
                className={`${styles.discoveryAns} ${openDiscovery === i ? styles.discoveryAnsOpen : ''}`}
              >
                {d.ans}
              </div>
            </button>
          ...
          <div className={styles.roiResults}>
            {[0, 1, 2].map((i) => (
              <div key={i} className={styles.roiResult}>
                <div className={styles.roiResultVal} style={{ color: sku.color }}>
                  {roiVals[i * 2]}
                </div>
