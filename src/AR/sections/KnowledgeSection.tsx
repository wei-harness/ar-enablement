    ...
    <div>
      ...
      >
        <div className={styles.masterCardInner}>
          <div className={styles.masterCardText}>
            ...
            <div className={styles.masterTitle}>ARMaster Knowledge Test</div>
            <div className={styles.masterDesc}>
              50 randomized questions drawn from all 5 SKU banks plus cross-SKU knowledge. Prove you
              know the full platform.
            </div>
          </div>
      ...
      <div className={styles.quizHubGrid}>
        {Object.entries(SKU_META).map(([k, m]) => (
          ...
          >
            <div className={styles.qhcIcon}>
              <CCMIcon name={m.icon} size={20} />
            </div>
            <div className={styles.qhcTitle}>{m.name}</div>
            <div className={styles.qhcDesc}>
              Test your knowledge on qualifying, positioning, and winning with {m.name}.
            </div>
            <div className={styles.qhcMeta}>
              <span
                className={styles.qhcBadge}
...
function ActiveQuiz({
  ...
  const skuLabel = isMaster ? 'ARMaster Test' : SKU_META[q.sku!]?.name || '';

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
        <button
          className={`${styles.quizBtn} ${styles.quizBtnGhost}`}
          style={{ fontSize: 12 }}
      ...
      <div className={styles.quizContainer}>
        <div className={styles.quizTopbar}>
          <div>
            ...
            <div className={styles.quizTitleBar}>
              {isMaster ? 'ARMaster Knowledge Test' : 'SKU Quiz'}
            </div>
          </div>
          <div className={styles.quizProgressWrap}>
            <div className={styles.quizScoreLive}>{state.score} correct</div>
            <div className={styles.quizProgBar}>
              <div className={styles.quizProgFill} style={{ transform: `scaleX(${pct / 100})` }} />
            </div>
            <div className={styles.quizProgTxt}>
              Q{state.current + 1} of {total}
            </div>
          </div>
        </div>
        ...
        <div className={styles.quizNav}>
          {answered && (
            <button
              className={`${styles.quizBtn} ${styles.quizBtnPrimary}`}
    ...
    <div>
      ...
      <div className={styles.quizContainer}>
        <div className={styles.quizResults}>
          <div className={styles.quizScoreBig}>
            {state.score}/{total}
          </div>
          <div className={styles.quizScoreLabel}>
            You answered {state.score} out of {total} questions correctly ({pct}%)
          </div>
          <div
            className={styles.quizGradeBadge}
            style={{
              background: gradeBg,
              color: gradeColor,
          ...
          <div className={styles.quizResultsList}>
            ...
            {state.questions.map((q, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`${styles.qrlItem} ${pass ? styles.qrlItemPass : styles.qrlItemFail}`}
                >
      ...
      <div>
        <div className={styles.sectionHdr}>
          ...
          <p className={styles.sectionSub}>
            Test your ARknowledge across all SKUs. 140+ questions, randomized each time.
          </p>
        </div>
