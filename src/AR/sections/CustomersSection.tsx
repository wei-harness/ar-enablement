    ...
    <div>
      <div className={styles.sectionHdr}>
        ...
        <p className={styles.sectionSub}>
          Health snapshots, checkpoint details, and lessons learned from our top ARaccounts.
        </p>
      </div>
      ...
      <div className={styles.customerNavGrid}>
        {CUSTOMERS.map((c, i) => (
          ...
          >
            <div
              className={styles.customerNavName}
              style={{ color: i === activeCustomer ? c.color : undefined }}
            >
              {c.name}
            </div>
      ...
      <div className={styles.statGrid} style={{ marginBottom: 20 }}>
        {customer.stats.map((s) => (
          <div key={s.l} className={styles.statCard}>
            <div className={styles.statVal} style={{ color: customer.color }}>
              {s.v}
            </div>
        ...
        <div className={styles.timelineTrack}>
          {customer.checkpoints.map((cp, i) => (
            <button
              ...
              style={{
                left:
                  customer.checkpoints.length > 1
                    ? `${(i / (customer.checkpoints.length - 1)) * 100}%`
                    : '50%',
                background: STATUS_COLOR[cp.status],
                border: 'none',
                cursor: 'pointer',
              }}
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        >
          What We Fixed Across All Accounts
        </h3>
      </div>
      ...
      <div className={styles.lessonLayout}>
        ...
        <div className={styles.lessonPanel}>
          <span
            ...
            style={{
              background: lesson.tagBg,
              color: lesson.tagColor,
              border: `1px solid ${lesson.tagBdr}`,
            }}
          >
            {lesson.tag}
          </span>
          ...
          <div className={styles.lessonPanelNav}>
            ...
            <span className={styles.lessonPanelNavCount}>
              {activeLessonIdx + 1} of {LESSONS_DATA.length}
            </span>
            <button
              className={styles.lessonPanelNavBtn}
      ...
      <div style={{ marginTop: 40 }}>
        ...
        <div className={styles.gwpGrid}>
          {GWP_IN_PROGRESS.map((g) => (
            <div
              key={g.name}
              className={styles.gwpItem}
              style={{ borderColor: 'color-mix(in srgb, var(--warning-amber) 25%, transparent)' }}
            >
              <div className={styles.gwpArea} style={{ color: 'var(--warn)' }}>
                {g.area}
              </div>
              <div className={styles.gwpName}>{g.name}</div>
              <div className={styles.gwpDesc}>{g.desc}</div>
